...
def calculateOpt(raw_list):
    ...
    output_list.append((row[0],len(raw_list),row[3],row[4],row[5],round(float(median_distance),2),median_small_shops,median_larger_shops,max(distances),round(avgFind(distances),2)))
...
for word in word_set:
    ...
    for word_score_shop in word_score_shop_list:
        if word==word_score_shop[0]:
            word_score_shop_wait_sort.append((word_score_shop[0],word_score_shop[1],word_score_shop[2],word_score_shop[3],word_score_shop[4],word_score_shop[5]))
    #do sort here
...
headers = ['word', 'cityNumber', 'word_central_lon', 'word_central_lat','radius', 'median' ,'shopNumberLessThanMedian','shopNumberLargerThanMedian','largestDistance','avgDistance']
